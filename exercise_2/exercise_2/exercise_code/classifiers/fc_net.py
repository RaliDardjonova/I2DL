...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        hidden, (fc_cache, relu_cache) = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        ...
        dh, dw2, db2 = affine_backward(dx, scores_cache)
        dw2 += self.reg * self.params['W2']
        grads['W2'] = dw2
        grads['b2'] = db2
        dh = relu_backward(dh, hidden)
        dx, dw1, db1 = affine_backward(dh, fc_cache)
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    dnorm, dgamma, dbeta = batchnorm_backward(da, batchnorm_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(2, self.num_layers):
            w_name = 'W' + str(i)
            self.params[w_name] = weight_scale * np.random.randn(hidden_dims[i-2], hidden_dims[i-1])
            b_name = 'b' + str(i)
            self.params[b_name] = np.zeros(hidden_dims[i-1])

        w_name = 'W' + str(self.num_layers)
        b_name = 'b' + str(self.num_layers)
        self.params[w_name] = weight_scale * np.random.randn(hidden_dims[self.num_layers-2], num_classes)
        ...
        if self.use_batchnorm:
            for i, dim in enumerate(hidden_dims):
                ...
                self.params[gamma_name] = np.ones(dim, dtype=float)
                self.params[beta_name] = np.zeros(dim, dtype=float)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_batchnorm:
                ...
                hidden[i], cache[i] = affine_batchnorm_relu_forward(hidden[i-1], self.params[w_name], self.params[b_name], self.params[gamma_name], self.params[beta_name], self.bn_params[i-1])
            else:
                hidden[i], cache[i] = affine_relu_forward(hidden[i-1], self.params[w_name], self.params[b_name])

        w_name = 'W' + str(self.num_layers)
        b_name = 'b' + str(self.num_layers)
        scores, scores_cache = affine_forward(hidden[self.num_layers-1], self.params[w_name], self.params[b_name])
        ...
        w_name = 'W' + str(self.num_layers)
        b_name = 'b' + str(self.num_layers)
        dh, dw, db = affine_backward(dx, scores_cache)
        ...
        for i in range(self.num_layers-1, 0, -1):
            ...
            #dx, dw, db = affine_relu_backward(dx, (fc_cache[i], relu_cache[i]))
            #dh = relu_backward(dx, hidden[i])
            #dx, dw, db = affine_backward(dh, fc_cache[i])
