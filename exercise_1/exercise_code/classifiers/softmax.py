...
def cross_entropy_loss_naive(W, X, y, reg):
    ...
    for sample, label in zip(X,y):
        ...
        # calculating loss
        predictions = sample.dot(W)
        predictions = predictions - np.max(predictions) # handle numeric instability

        softmax_numer = np.exp(predictions)
        softmax_denom = np.sum(softmax_numer)
        softmax = softmax_numer/softmax_denom
        curr_loss = softmax[label]
        loss += -np.log(curr_loss)

        #calculating gradient
        for j in range(num_classes):
            if j == label:
                dW[:,j] += (softmax[j] - 1)*sample
            else:
                dW[:,j] += (softmax[j])*sample
...
def softmax_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    best_softmax = None
    all_classifiers = []
    learning_rates = [1e-7, 2e-7, 3e-7, 5e-7, 6e-7, 7e-7,8e-7]
    regularization_strengths = [0.5e4,1e4, 2e4, 2.5e4, 3e4, 5e4,6e4,7e4]
    ...
    for rate in learning_rates:
        for reg_str in regularization_strengths:
            ...
            loss_hist = softmax.train(X_train, y_train, learning_rate=rate, reg=reg_str,
